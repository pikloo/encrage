...
<figure class="<?php if (isset($is_home)  || isset($is_member_page)) echo 'reveal' ?> relative group xl:overflow-hidden break-inside">
    <?php if (has_post_thumbnail(get_the_ID())) : ?>
        <a aria-label="Se rendre sur la série <?= the_title(); ?> " href="<?= esc_url(get_permalink()); ?>"><img class="w-full h-72  object-cover" src="<?= esc_url(get_the_post_thumbnail_url(get_the_ID(), 'medium_large')) ?>" alt="<?= the_title(); ?>" /></a>
    <?php endif; ?>
    <figcaption class="xl:hidden xl:absolute xl:group-hover:block xl:bottom-0 xl:text-white xl:group-hover:animate-slideInUp">
        <div class="xl:bg-black xl:px-4 xl:text-xl xl:w-fit">
            ...
            <?php if (!isset($is_member_page) || !$is_member_page) :  ?>
                <a aria-label="Se rendre sur la page de <?= esc_html(get_the_title(get_post_meta(get_the_ID(), 'photographer', true))); ?>" href="<?= esc_url(get_permalink(get_post_meta(get_the_ID(), 'photographer', true))); ?>">
